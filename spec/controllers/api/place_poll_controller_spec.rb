...
RSpec.describe Api::PlacePollController, type: :controller do
  ...
  describe 'PUT #choose_place' do
    context 'valid request' do
      it 'should set the place as chosen place for the event' do
        user = create(:user)
        user2 = create(:user)
        event = create(:event, creator: user)
        event.participants << user2
    ...
    context 'bad request' do
      ...
      it 'should return 404 when the event does not exist' do
        put :choose_place, params: {hash: 'wrong', id: @place_poll_option.id}
        expect(response).to have_http_status(404)
      end

      it 'should return 404 when the place poll option does not exist' do
        put :choose_place, params: {hash: @event.event_hash, id: @place_poll_option.id + 1}
        expect(response).to have_http_status(404)
      end
    end
  ...
  describe 'DELETE #detroy_chosen_place' do
    context 'valid request' do
      before do
        user = create(:user)
        @event = create(:event, creator: user)
      ...
      it 'should delete the event\'s chosen place' do
        delete :destroy_chosen_place, params: {hash: @event.event_hash}

        expect(response).to have_http_status(200)
        @event.reload
        expect(@event.chosen_place).to be_nil
      end

      it 'should refuse to delete the chosen place because event has no place poll' do
        @event.has_place_poll = false
        @event.save
        delete :destroy_chosen_place, params: {hash: @event.event_hash}
    ...
    context 'bad request' do
      ...
      it 'should return 404 when the event does not exist' do
        ...
        delete :destroy_chosen_place, params: {hash: 'wrong'}
        expect(response).to have_http_status(404)
      end
    end
